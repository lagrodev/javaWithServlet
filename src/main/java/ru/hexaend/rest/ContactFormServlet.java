 ...
 *
 * <p>GET — отображает пустую форму (создание) или заполненную (редактирование, если передан {@code
 * id}).<br>
 * POST — сохраняет данные формы: создаёт новый контакт или обновляет существующий.
 *
...
public class ContactFormServlet extends HttpServlet {
   ...
   *
   * <p>Параметры: {@code id} (опц.), {@code firstName}, {@code lastName}, {@code phones} (через
   * запятую). Если {@code id} передан — обновление, иначе — создание нового контакта.
   */
  @Override
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    final List<String> phones =
            Arrays.stream(request.getParameter("phones").split(","))
                    .map(String::trim)
                    .filter(p -> !p.isBlank())
                    .toList();

    if (rawId != null && !rawId.isBlank()) {
      getService().editContact(UUID.fromString(rawId), firstName, lastName, phones);
      response.sendRedirect("/contacts");
    } else {
      getService().addContact(firstName, lastName, phones);
      response.sendRedirect("/contacts");
    }
  }
}

 ...
 *
 * <p>Реализует {@code equals}/{@code hashCode} на основе идентификатора. Все доменные объекты
 * приложения наследуются от этого класса.
 *
...
public class AbstractEntity<ID> {
  ...
  /**
   * Создаёт сущность с указанным идентификатором. Метки времени инициализируются текущим моментом.
   *
  ...
  /**
   * Создаёт сущность с явно заданными метками времени. Используется при восстановлении объекта из
   * БД.
   *
   * @param id        уникальный идентификатор, не {@code null}

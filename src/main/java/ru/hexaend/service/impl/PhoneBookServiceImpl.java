 ...
 *
 * <p>Выполняет валидацию телефонных номеров через {@link PhoneValidator} и делегирует персистентные
 * операции в {@link ContactRepository}. При отсутствии контакта выбрасывает {@link
 * ContactNotFoundException}, при невалидных данных — {@link ValidationException}.
 *
...
public class PhoneBookServiceImpl implements PhoneBookService {
  ...
  public Contact addContact(String firstName, String lastName, List<String> phoneNumbers) {
    ...
    final Contact contact = new Contact(firstName, lastName, phoneNumbers);
  ...
  public void deleteContact(UUID contactId) {
    contactRepository.findById(contactId).orElseThrow(() -> new ContactNotFoundException("Contact with id " + contactId + " not found"));
    contactRepository.deleteById(contactId);
  }

  @Override
  public Contact editContact(UUID contactId, String firstName, String lastName, List<String> phoneNumbers) {
    final Contact contact = contactRepository.findById(contactId).orElseThrow(() -> new ContactNotFoundException("Contact with id " + contactId + " not found"));
    validatePhoneNumber(phoneNumbers);
  ...
  public Contact addPhoneNumber(UUID contactId, String phoneNumber) {
    ...
    if (!phoneValidator.isValid(phoneNumber)) {
      throw new ValidationException("Phone number " + phoneNumber + " is invalid");
    }
  ...
  public Contact getContactById(UUID contactId) {
    return contactRepository.findById(contactId).orElseThrow(() -> new ContactNotFoundException("Contact with id " + contactId + " not found"));
  }

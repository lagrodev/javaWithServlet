 ...
 *
 * <p>Маппит только скалярные поля контакта (id, имя, фамилия, метки времени).
 * Список телефонных номеров не заполняется — он загружается отдельным запросом
 * или через JOIN в вызывающем коде.</p>
 *

 ...
 *
 * <p>Реализует паттерн Dynamic Proxy: перехватывает вызовы методов интерфейса репозитория,
 * извлекает SQL из аннотации {@link Query}, маппер из {@link Mapper} и делегирует выполнение в
 * {@link QueryExecutor}.
 *
 * <p>Тип возвращаемого значения метода определяет стратегию выполнения: {@code List} — queryList,
 * {@code Optional} — queryOne, {@code int/Integer} — update, {@code void} — update без возврата.
 *
...
public class RepositoryInvocationHandler implements InvocationHandler {
   ...
   *
   * <ol>
   *   <li>Методы {@link Object} (toString, equals, hashCode) делегируются напрямую.
   *   <li>Извлекаются аннотации {@link Query} и {@link Mapper} с вызванного метода.
   *   <li>По типу возвращаемого значения определяется стратегия выполнения ({@code List}, {@code
   *       Optional}, {@code int/void}) через enum {@link ReturnedType}.
   *   <li>Запрос выполняется через {@link QueryExecutor} с соответствующим маппером.
   * </ol>
   *
  ...
  public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    if (mapperAnnotation == null) {
      throw new UnsupportedOperationException(
              "Метод "
                      + method.getName()
                      + " не аннотирован @Mapper, нужно указать класс Маппера для Маппинга"
                      + " результата запроса");
    }
